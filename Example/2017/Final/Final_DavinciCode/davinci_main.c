...
//#endif
	...
	imgBase = cvCreateImage(cvSize(RESIZE_WIDTH, RESIZE_HEIGHT), IPL_DEPTH_8U, 1);	// BGR 
	imgBase2 = cvCreateImage(cvSize(RESIZE_WIDTH, RESIZE_HEIGHT), IPL_DEPTH_8U, 1);	// BGR 
	imgDiff = cvCreateImage(cvSize(RESIZE_WIDTH, RESIZE_HEIGHT), IPL_DEPTH_8U, 1);	// BGR 
	cvCvtColor(imgOrigin, imgBase2, CV_RGB2GRAY);
	...
	unsigned char current_sequence = sequence;
...
#ifdef DISTANCE_TEST	
		if(isInBoundryValue(1, 550, 4095)) setVehicleSpeed(50);
		else setVehicleSpeed(0);
#endif
...
/*			if (sequence == 1 && my_timer > 100 && !Misson_HillDrive_Done) // When StopSign detected!
			{
				vehicleStop();
				sleep(2);
				setVehicleDirection_Value(1500);
				positionControl(-100, -25);
				sleep(2);
				reset_Mtimer = 1;
				while(my_timer < HILL_ACC_TIME_100MS) setVehicleSpeed(SPEED_HILL_ASC);
				while(my_timer < HILL_DSC_TIME_100MS) setVehicleSpeed(SPEED_HILL_DESC);
				Misson_HillDrive_Done = 1;
			}
*/
